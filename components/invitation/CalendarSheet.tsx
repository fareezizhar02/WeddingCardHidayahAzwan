...
export default function CalendarSheet({ isOpen, onClose }: CalendarSheetProps) {
  // Event details
  const title = "Majlis Perkahwinan Hidayah & Azwan";
  const location =
    "Dara Glass Hall, Pt 1046, Lorong Pengkalan Penambang, Kampung Pendek, 15100 Kota Bharu, Kelantan";
  ...
  const displayDate = "Sabtu, 28 Ogos 2026";
  const displayTime = "8:00 PM - 12:00 AM";
  ...
  const downloadAppleICS = () => {
    const ics = [
      ...
      "METHOD:PUBLISH",
      "BEGIN:VEVENT",
      `UID:wedding-hidayah-azwan-${Date.now()}@weddingcard`,
      `DTSTAMP:${toICSDateTimeUTC(new Date())}`,
      `SUMMARY:${escapeICS(title)}`,
      `DESCRIPTION:${escapeICS(description)}`,
      `LOCATION:${escapeICS(location)}`,
      `DTSTART;TZID=${timezone}:20260328T200000`,
      `DTEND;TZID=${timezone}:20260329T000000`,
      "STATUS:CONFIRMED",
      "SEQUENCE:0",
      "END:VEVENT",
      "END:VCALENDAR",
    ...
    link.href = url;
    link.download = "Hidayah-Azwan-Wedding.ics";
